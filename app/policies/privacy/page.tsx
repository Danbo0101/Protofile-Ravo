...
export default function PrivacyPolicyPage() {
    ...
    return (
        ...
        >
            <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 py-14 md:py-20">
                {/* Breadcrumb + Hero */}
                <motion.div
                    variants={floatIn}
                    initial="hidden"
                ...
                >
                    <nav className="text-xs uppercase tracking-[0.18em] text-neutral-500">
                    ...
                    <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-neutral-200/80 bg-white/70 backdrop-blur px-4 py-1.5 text-[11px] tracking-[0.2em] uppercase text-neutral-600">
                        Legal
                    </div>
                    ...
                    <div className="mt-6 relative">
                        <GradientHairline />
                        <div className="rounded-2xl border border-neutral-200/80 bg-white/70 backdrop-blur px-5 md:px-6 py-5 md:py-6 text-[15px] text-neutral-800 leading-relaxed">
                ...
                {/* Body: TOC + Content */}
                <div className="grid gap-10 lg:grid-cols-[290px,1fr]">
                    {/* TOC */}
                    <motion.aside
                        variants={fadeIn}
                        initial="hidden"
                    ...
                    >
                        <div className="rounded-2xl overflow-hidden border border-neutral-200/80 bg-white/70 backdrop-blur">
                            <div className="px-5 py-4 border-b border-neutral-200/80">
                            ...
                            <ul className="p-2">
                                {TOC.map((item) => (
                                    <li key={item.id}>
                                        <a
                                            href={`#${item.id}`}
                                            className="group flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-100/80"
                                        >
                                            <span className="size-1.5 rounded-full bg-neutral-300 group-hover:bg-neutral-900 transition-colors" />
                    ...
                    <article className="max-w-3xl">
                        ...
                        <Section id="contact" title="Contact">
                            <div className="rounded-xl border border-neutral-200/80 bg-white/70 backdrop-blur p-5 text-[15px] leading-relaxed">
                        ...
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            transition={{ duration: 0.5, ease: EASE_OUT }}
                            className="mt-10 text-xs text-neutral-500"
                        >
...
function MetaPill({ label, value }: { label: string; value: string }) {
    return (
        <div className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white/70 px-3 py-1">
    ...
    return (
        ...
        >
            <div className="mb-5 flex items-center gap-3">
                <h2 className="font-serif text-2xl md:text-[28px] leading-tight tracking-tight">
                    {title}
                </h2>
                {badge ? (
                    <span className="rounded-full border border-neutral-200 bg-white/70 px-2.5 py-0.5 text-[11px] uppercase tracking-[0.18em] text-neutral-600">
                        {badge}
                    </span>
                ) : null}
            </div>
            <div className="prose prose-neutral max-w-none prose-p:leading-relaxed prose-ul:my-2 prose-li:leading-relaxed">
                {children}
            </div>
            <Divider />
        </motion.section>
...
function Divider() {
    return (
        <div className="my-8 h-px w-full bg-[linear-gradient(to_right,transparent,rgba(0,0,0,0.15),transparent)]" />
    );
}

function UL({ children }: { children: React.ReactNode }) {
    return (
        <ul className="marker:text-neutral-400 list-disc pl-5 space-y-2 text-[15px]">
            {children}
        </ul>
    );
}
